...
class Discriminator_VGG_scale(nn.Module):
    def __init__(self, in_chl, nf, scale):
        super(Discriminator_VGG_scale, self).__init__()
        # in: [in_chl, 48 * scale, 48 * scale]
        self.conv0_0 = nn.Conv2d(in_chl, nf, 3, 1, 1, bias=True)
        self.conv0_1 = nn.Conv2d(nf, nf, 4, scale, 1, bias=False)
        self.bn0_1 = nn.BatchNorm2d(nf, affine=True)
        # [nf, 48, 48]
        self.conv1_0 = nn.Conv2d(nf, nf * 2, 3, 1, 1, bias=False)
        self.bn1_0 = nn.BatchNorm2d(nf * 2, affine=True)
